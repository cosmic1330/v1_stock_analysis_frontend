        ...
        getData(){
            Get_One_Month_Of_Stock(3380)
                .then(response => {
                    ...
                    let close_price = response.data.map(stock=>{return parseInt(stock.close_price);});
                    let UP = response.data.map(stock=>{return this.customsPrices(stock,"UP");});
                    let Mid = response.data.map(stock=>{return this.customsPrices(stock,"Mid");});
                    let Down = response.data.map(stock=>{return this.customsPrices(stock,"Down");});
                    let date = response.data.map(stock=>{return stock.date;});
                    console.log(date);
                    this.chartOptions.series[0].data =  close_price;
                    this.chartOptions.series[1].data =  UP;
                    this.chartOptions.series[2].data =  Mid;
                    this.chartOptions.series[3].data =  Down;
