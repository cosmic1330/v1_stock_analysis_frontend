    ...
    <div id="cards" v-if="!loading">
        <el-table :data="Object.keys(stocks)" style="width: 100%">
            ...
            <el-table-column label="三關價" width="100">
                <template slot-scope="scope">
                    <p>上關: {{ customsPrices(stocks[scope.row][stocks[scope.row].length-1], "UP") }}</p>
                    <p>中關: {{ customsPrices(stocks[scope.row][stocks[scope.row].length-1], "Mid") }}</p>
                    <p>下關: {{ customsPrices(stocks[scope.row][stocks[scope.row].length-1], "Down") }}</p>
                </template>
            </el-table-column>
            ...
            <el-table-column type="data" label="威廉指數">
                <template slot-scope="scope">
                    {{ stocks[scope.row][stocks[scope.row].length-1]['w14'] }}
                </template>
            </el-table-column>
        </el-table>
